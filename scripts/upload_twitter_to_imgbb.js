/**
 * Upload all 104 Twitter/X images to imgbb
...
// Paths
const IMAGES_DIR = 'C:\\Users\\MarieLexisDad\\Modelit-Twitter\\images';
const OUTPUT_FILE = 'C:\\Users\\MarieLexisDad\\scripts\\twitter_imgbb_urls.json';
 ...
 */
async function main() {
  console.log('🐦 Twitter/X Image Upload to imgbb');
  console.log('='.repeat(70));

  // Get all image files
  const imageFiles = fs.readdirSync(IMAGES_DIR)
    .filter(f => f.match(/Twitter_Post_\d+_Image\.png/))
    .sort();

  console.log(`Found ${imageFiles.length} Twitter/X images\n`);
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    const postNumber = parseInt(filename.match(/\d+/)[0]);
